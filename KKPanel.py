...
class PlaceholderProperties(PropertyGroup):
    ...
    blender_path: StringProperty(default = bpy.context.preferences.addons[__package__].preferences.blender_path)
    ...
    def draw(self,context):
        ...
        if scene.blender_path:
            ...
            if not c.has_blender_dependency():
                #if it is not there, draw a button asking the user to download it
                box = layout.box()
                col = box.column(align=True)
                row = col.row(align=True)
                row.alignment = 'EXPAND'
                textTowrap = t('install_dependency_tt')
                ...
                for text in wList: 
                    ...
                    row.label(text=text)
                row = col.row(align=True)
                row.operator('kkbp.installdependency', text = t('install_dependency'), icon='MODIFIER')
