...
class modify_armature(bpy.types.Operator):
    ...
    def set_bone_roll_data(self):
        '''Set each bone's rotation'''
        if roll_data := c.json_file_manager.get_json_file('KK_BoneRotation.json'):
            armature = c.get_armature()
            c.switch(armature, 'edit')
            for bone_info in roll_data:
                if armature.data.edit_bones.get(bone_info['boneName']):
                    armature.data.edit_bones[bone_info['boneName']].roll = bone_info['rotation']
            c.print_timer('set_bone_roll_data')
        else:
            self.set_bone_roll_data_old()  # compatible mode
        
    def set_bone_roll_data_old(self):
