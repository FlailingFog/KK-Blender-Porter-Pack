...
translation_dictionary = {
    ...
    'cat_drop_B_tt' : "すべてをインポートしてインポートをポーズ。ポーズ状態に手動で服を別々になれる。髪の毛は別のオブジェクトに別々にしてから新しいオブジェクトは「Hair」か「hair」に名前を変更しないと。服を手動でカテゴライズしたら「カテゴライズしまって」ボタンをクリックして。別の服はひとりでに隠される",
