translation_dictionary = {
    ...
    'install_dependency_tt': """このボタンをクリックすると、Blender 2.90が自動的にダウンロードされる。このBlenderバージョンは、KKBPモデルをインポートするために必要です。
このインストールは、https://download.blender.org/release/ から193MBをダウンロードされる。
インストールの後は504MBがかかる。データはKKBPアドオンフォルダーに保存される。KKBPアドオンをアンインストールすると504MBのデータが削除される。
